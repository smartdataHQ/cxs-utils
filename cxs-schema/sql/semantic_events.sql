-- Extends segment.com semantic event schema and is tailored for the Context Suite
-- This table is used to store semantic events that are generated by the Context Suite
...
CREATE TABLE IF NOT EXISTS default.semantic_events (
    ...
    `base_events` Nested (                                          -- If this is a derived event (higher order) then this will be populated with the base event information
        `event_gid` UUID,                                           -- The event gid of the root event
        `type` LowCardinality(String),                              -- The type of the event (e.g. "page", "track", "identify", "group", "alias", "screen", "commerce")
        `event` LowCardinality(String),                             -- The name of the event (e.g. "Page Viewed", "Product Added", "User Signed Up")
        `timestamp` DateTime64,                                     -- The timestamp of the event in UTC (e.g. "2022-01-01 00:00:00")
        `message_id` String,                                        -- The message ID of the base event
        `entity_gid` UUID                                           -- The entity GID of the event
    ),
    ...
    `root_event_gid` UUID,                                          -- Teh root event GID of the event, if this is a derived event (higher order) then this will be populated with the root event GID
